...
QCamxHAL3TestDevice::QCamxHAL3TestDevice(
    camera_module_t* module, int cameraId, QCamxHAL3TestConfig* Config, int mode) :
    mModule(module),
    mCameraId(cameraId),
    mConfig(Config)
{
...
************************************************************************/
void QCamxHAL3TestDevice::CallbackOps::ProcessCaptureResult(const camera3_callback_ops *cb,const camera3_capture_result *result)
{
...
void QCamxHAL3TestDevice::configureStreams(std::vector<Stream*> streams, int opMode)
{
    ...
    if (mConfig && mConfig->mRangeMode != -1 && mConfig->mImageType != -1) {
        ...
        opMode = (opMode) | ((mConfig->mRangeMode * 5 + mConfig->mImageType + 1) << 16) | (0x1 << 24);
    }
    ...
    CameraMetadata::getTagFromName("org.codeaurora.qcamera3.sessionParameters.availableStreamMap",vTags.get(), &tag);
    ...
    for (uint32_t i = 0; i < streams.size(); i++) {
        ...
        if (streams[i]->pstream->format == HAL_PIXEL_FORMAT_BLOB) {
            ...
            bufferManager->AllocateBuffers(
                ...
                streams[i]->pstream->usage,
                streams[i]->pstream->usage, streams[i]->type, subformat);
        } else {
            bufferManager->AllocateBuffers(
                StreamBufferMax,
                streams[i]->pstream->width,
                streams[i]->pstream->height,
                (int32_t)(streams[i]->pstream->format),
...
************************************************************************/
void QCamxHAL3TestDevice::constructDefaultRequestSettings(int index,camera3_request_template_t type,
    bool useDefaultMeta)
{
    // construct default
        ...
        }
        {
            ...
            if ((0 == res) && (entry.count > 0)) {
                ...
                QCAMX_PRINT("AECOMP ae_comp_range min = %d, max = %d\n",
                    mConfig->mAECompRangeMin, mConfig->mAECompRangeMax);
            }
            else {
...
************************************************************************/
int QCamxHAL3TestDevice::ProcessOneCaptureRequest(int* requestNumberOfEachStream,int* frameNumber)
{
        ...
        if (empty && !hasRequest) {
            ...
            case REQUEST_CHANGE: {
                ...
                for (int i = 0;i < device->mStreams.size();i++) {
                    if (msg->mask & (1<<i)) {
                        (thData->requestNumber[i] == REQUEST_NUMBER_UMLIMIT) ? thData->requestNumber[i] =
                            msg->requestNumber[i] : thData->requestNumber[i] += msg->requestNumber[i];
                    }
                }
                pthread_mutex_unlock(&thData->mutex);
                delete msg;
...
int QCamxHAL3TestDevice::getJpegBufferSize(uint32_t width, uint32_t height)
{
    ...
    if (jpegBufMaxSize.count == 0) {
        QCAMX_ERR("Camera %d: Can't find maximum JPEG size in static metadata!",
            mCameraId);
        return 0;
    }
    ...
    float scaleFactor = (1.0f * width * height) /
        ((maxJpegBufferSize - sizeof(camera3_jpeg_blob))/1.5f);
    ...
    for (int i = 0; i < entry.count; i+=4) {
        if (entry.data.i32[i + 3] ==
            ...
            if (ValidStream == nullptr) {
                AvailableStream stream = {entry.data.i32[i+1],
                                          entry.data.i32[i+2],
                                          entry.data.i32[i]};
