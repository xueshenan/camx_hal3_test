        ...
        if (NULL != mBuffers[i])
        {
            munmap(mBufferinfo[i].vaddr, mBufferinfo[i].size);
                ...
                native_handle_t* pHandle = const_cast<native_handle_t*>(
                                                        static_cast<const native_handle_t*>(mBuffers[i]));
                if (pHandle->data[1] != 0 || pHandle->data[2] != 0)
                {
                    ...
                    close(buf_fd);
                    QCamxHal3TestGBM::GetHandle()->FreeGbmBufferObj(reinterpret_cast<struct gbm_bo*>
                                                                    (bo_handle));
                }
                native_handle_close((native_handle_t *)mBuffers[i]); //
                native_handle_delete((native_handle_t *)mBuffers[i]);
...
************************************************************************/
int QCamxHAL3TestBufferManager::AllocateBuffers(
    uint32_t numBuffers,
    uint32_t width,
    uint32_t height,
    uint32_t format,
...
)
{
    ...
    {
        AllocateOneBuffer(width, height, format, producerFlags, consumerFlags, &mBuffers[i], &mBufferStride, i, type, subformat);
        mNumBuffers++;
...
************************************************************************/
int QCamxHAL3TestBufferManager::AllocateOneBuffer(
    uint32_t           width,
    uint32_t           height,
    uint32_t           format,
...
#if defined USE_GRALLOC1
    result = AllocateOneGralloc1Buffer(width,
                                       height,
                                       format,
                                       producerUsageFlags,
                                       consumerUsageFlags,
...
#elif defined USE_ION
    result = AllocateOneIonBuffer(width,
                                  height,
                                  format,
                                  producerUsageFlags,
                                  consumerUsageFlags,
                                  ...
                                  index, type, subformat);
#elif defined USE_GBM
    result = AllocateOneGbmBuffer(width,
                                  height,
                                  format,
                                  producerUsageFlags,
                                  consumerUsageFlags,
    ...
    Implsubformat      subformat)
{
    ...
    QCAMX_INFO("Alloc buffer fd:%d vaddr:%p len:%d width:%d height:%d stride:%d slice:%d format 0x%x\n",
        mBufferinfo[index].fd,
        mBufferinfo[index].vaddr,
        mBufferinfo[index].size,
        mBufferinfo[index].width,
        mBufferinfo[index].height,
        mBufferinfo[index].stride,
        mBufferinfo[index].slice,
        mBufferinfo[index].format);
...
************************************************************************/
int QCamxHAL3TestBufferManager::AllocateOneIonBuffer(
    uint32_t           width,
    uint32_t           height,
    uint32_t           format,
    uint64_t           producerUsageFlags,
    ...
    Implsubformat      subformat)
{
    ...
    switch (format) {
        ...
        case HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED:
        {
            ...
            if ((CameraTitanSocSDM865 == m_socId)  ||
                (CameraTitanSocSM7250 == m_socId)  ||
                (CameraTitanSocQSM7250 == m_socId) ||
                (CameraTitanSocQRB5165 == m_socId) ||
                (CameraTitanSocQRB5165N == m_socId)||
                (CameraTitanSocQCS7230 == m_socId) ||
                (CameraTitanSocQRB3165 == m_socId) ||
                (CameraTitanSocQRB3165N == m_socId)) {
...
#ifndef TARGET_ION_ABI_VERSION
    mBufferinfo[index].vaddr     = mmap(NULL, alloc.len, PROT_READ | PROT_WRITE, MAP_SHARED, ion_info_fd.fd, 0);
    mBufferinfo[index].fd        = ion_info_fd.fd;
#else
    mBufferinfo[index].vaddr     = mmap(NULL, alloc.len, PROT_READ | PROT_WRITE, MAP_SHARED, alloc.fd, 0);
...
* function: Allocate One Buffer from Ion interface
************************************************************************/
int QCamxHAL3TestBufferManager::AllocateOneGbmBuffer(
    uint32_t           width,
    uint32_t           height,
    uint32_t           format,
    uint64_t           producerUsageFlags,
    ...
    Implsubformat      subformat)
{
    ...
    pGbmBuffObject = QCamxHal3TestGBM::GetHandle()->AllocateGbmBufferObj(width,
                                                                           height,
                                                                           format,
                                                                           producerUsageFlags,
                                                                           consumerUsageFlags);
...
uint32_t QCamxHal3TestGBM::GetGbmUsageFlag(uint32_t gbm_format, uint32_t cons_usage, uint32_t prod_usage)
{
    uint32_t gbm_usage  = 0;
    for (auto &it : GBMUsageMap) {
        if (it.first & cons_usage ||
            it.first & prod_usage) {
            gbm_usage |= it.second;
        }
    }
...
struct gbm_bo* QCamxHal3TestGBM::AllocateGbmBufferObj(uint32_t              width,
                                                        uint32_t              height,
                                                        uint32_t              format,
                                                        uint64_t              producerUsageFlags,
                                                        uint64_t              consumerUsageFlags)
{
    ...
    uint32_t gbm_usage              = QCamxHal3TestGBM::GetGbmUsageFlag(gbm_format, consumerUsageFlags, producerUsageFlags);
    ...
    QCAMX_INFO("Format: %x => width: %d height:%d stride:%d ",
                                        gbm_bo_get_format(pGbmBuffObject),
                                        gbm_bo_get_width(pGbmBuffObject),
                                        gbm_bo_get_height(pGbmBuffObject),
