    ...
    while (*order != '\0' && !errfnd) {
        ...
        case MANUAL_EXPOSURE_COMP:{
            ...
            if(mAECompRangeMin <= aecomp && aecomp <= mAECompRangeMax)
            {
                ...
                QCAMX_PRINT("AECOMP setting ANDROID_CONTROL_AE_EXPOSURE_COMPENSATION value to :%d\n",aecomp);
            }
            else
            {
                QCAMX_PRINT("AECOMP ANDROID_CONTROL_AE_EXPOSURE_COMPENSATION value %d out of range | expected range %d - %d",aecomp, mAECompRangeMin, mAECompRangeMax);
            }
            break;
        }
        ...
        case COMPENSATION_RATIO:{
            ...
            CameraMetadata::getTagFromName("org.codeaurora.qcamera3.compensation_ratio.ratio",vTags.get(), &tag);
