...
void QCamxHAL3TestVideo::HandleMetaData(DeviceCallback* cb, camera3_capture_result *result)
{
    ...
    if (result->partial_result >= 1)
    {
       ...
       if (mConfig->mMetaDump.SATActiveArray)
       {
          ...
          if (str == 4) {
             QCAMX_INFO("3 Streams: frame_number: %d, SAT ActiveSensorArray: [%d,%d,%d,%d]", result->frame_number, activearray[0], activearray[1], activearray[2], activearray[3]);
             for (str = 0; str<4; str++) {
       ...
       if (mConfig->mMetaDump.SATCropRegion)
       {
          ...
          if (str == 4) {
              QCAMX_INFO("3 Streams: frame_number: %d, SAT ScalerCropRegion: [%d,%d,%d,%d]", result->frame_number, cropregion[0], cropregion[1], cropregion[2], cropregion[3]);
              for (str = 0; str<4; str++) {
    ...
    if (result->partial_result > 1)
    {
       if (mConfig->mMetaDump.rawsize)
       {
          ...
          int height       = 0;
          uint32_t raw_tag = 0;
          CameraMetadata::getTagFromName("com.qti.chi.multicamerainfo.MasterRawSize", vTags.get(), &raw_tag);
          res = find_camera_metadata_ro_entry(result->result, raw_tag, &entry);
          if ((0 == res) && (entry.count > 0)) {
              width  = entry.data.i32[0];
              height = entry.data.i32[1];
              QCAMX_INFO("frame_number: %d, multicamerainfo MasterRawSize:%dx%d, res:%d", result->frame_number, width, height, res);
          }
       }
    }
}
...
int QCamxHAL3TestVideo::PreinitStreams()
{
    ...
    if (mVideoMode <= VIDEO_MODE_HFR60) {
        ...
        if(mConfig->mRawStreamEnable == TRUE)
        {
            ...
            if ((0 != mConfig->mRawStream.width) &&
                (0 != mConfig->mRawStream.height))
            {
