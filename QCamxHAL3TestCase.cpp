...
************************************************************************/
void QCamxHAL3TestCase::DumpFrame(BufferInfo* info, unsigned int frameNum, StreamType dumpType, Implsubformat subformat )
{
    char fname[256];
    time_t timer;
    time(&timer);
    ...
    switch (format) {
        ...
        case HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED:
        {
            ...
            for (int idx = 0; idx < plane_cnt; idx++) {
                ...
                Plane[idx].width           = (localWidth / pTileInfo->BPPDenominator) * pTileInfo->BPPNumerator;
                float local1               = static_cast<float>(static_cast<float>(Plane[idx].width) / pTileInfo->widthBytes) / 64;
                Plane[idx].metadataStride  = static_cast<uint32_t>(ceil(local1)) * 1024;
                float local2               = static_cast<float>(static_cast<float>(Plane[idx].height) / pTileInfo->height) / 16;
                unsigned int localMetaSize = static_cast<uint32_t>(ceil(local2)) * Plane[idx].metadataStride;
                ...
                Plane[idx].pixelPlaneSize  = ALIGN((Plane[idx].planeStride * Plane[idx].sliceHeight), 4096);
...
void QCamxHAL3TestCase::showFPS(StreamType streamType)
{
    ...
    if (diff > s2ns(1)) {
        fps = (((double)((*frameCount) - (*lastFrameCount))) *
                (double)(s2ns(1))) / (double)diff;
        QCAMX_INFO("CAMERA %d, %s: %.4f, frameCount %d",
            mConfig->mCameraId, tag, fps, (*frameCount));
        (*lastFpsTime) = now;
...
void QCamxHAL3TestCase::init(camera_module_t* module, QCamxHAL3TestConfig* config)
{
    ...
    sp<VendorTagDescriptor> vTags =
        android::VendorTagDescriptor::getGlobalVendorTagDescriptor();
    CameraMetadata::getTagFromName(
        "com.qti.chi.temperature.temperature",
        vTags.get(), &mTagIdTemperature);
